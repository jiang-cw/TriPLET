...
DenNet = Denoising_Net(opt)
RecNet =  Reconstruction_Net(opt)
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    for batch_idx, (data, label, filename) in enumerate(train_loader):
        ...
        # Second, G(A) = B
        loss_g_l1 = L_F(fake_b, real_b) * opt.lamb
        loss_g_l2 = L_P(fake_b, real_b) * opt.lamb
        # loss_g_l1 = criterionMSE(fake_b, real_b) * opt.lamb

        generator_total_loss = loss_g_gan + loss_g_l1 + loss_g_l2
    ...
    if epoch % opt.save_fre == 0:
        ...
        torch.save(DenNet.state_dict(), '%s/g_epoch_{}.pth'.format(epoch) % save_path)
        torch.save(RecNet.state_dict(), '%s/g_epoch_{}.pth'.format(epoch) % save_path)
